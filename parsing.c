...
lval* lval_call(lenv* e, lval* f, lval* a) {
    ...
    // If '&' remains bind args symbol to empty list
    if (f->formals->count > 0 &&
        ...
        lval_del(lval_pop(f->formals, 0)); // '&' isnt needed
...
int main(int argc, char** argv) {
    ...
    if (argc == 1) { // No files given so run interactive prompt
    ...
    if (argc >= 2) { // load and run the specified files in order
